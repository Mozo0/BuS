...
class SKNet(nn.Module):
    ...
    def __init__(self, in_channel=3, nums_class=1000, block=SKBlock, nums_block_list=[3, 4, 6, 3]):
        super(SKNet, self).__init__()
        self.inplanes = 64
        # in_channel=3  out_channel=64  kernel=7x7 stride=2 padding=3
        self.conv = nn.Sequential(nn.Conv2d(in_channel, 64, 7, 2, 3, bias=False),
                                  nn.BatchNorm2d(64),
                                  nn.ReLU(inplace=True))
...
def SKNet50(nums_class=2, in_channel=3):
    return SKNet(in_channel, nums_class, SKBlock, [3, 4, 6, 3])  # 论文通过[3, 4, 6, 3]搭配出SKNet50


def SKNet101(nums_class=2, in_channel=3):
    return SKNet(in_channel, nums_class, SKBlock, [3, 4, 23, 3])


if __name__ == '__main__':
    x = torch.rand(5, 1, 224, 224)
    model = SKNet50(in_channel=1)
    y = model(x)
    print(y.shape)
